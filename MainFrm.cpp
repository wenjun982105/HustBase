...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext)
{
	...
	if (!m_wmSplitter2.CreateView(0, 1, RUNTIME_CLASS(CTableList), CSize(4 * eRect.Width(), 2* eRect.Height()/3), pContext))//��ʾ��Ϣ
	{
		return FALSE;
	}
	if (!m_wmSplitter1.CreateView(0, 0, RUNTIME_CLASS(CEditArea), CSize(eRect.Width(), eRect.Height()/6), pContext))	//��״�ؼ�
	{
		return FALSE;
	}
	if (!m_wmSplitter1.CreateView(2, 0, RUNTIME_CLASS(CHustBaseView), CSize(eRect.Width(), eRect.Height()/6), pContext))	//��״�ؼ�
	{
		return FALSE;
	}
